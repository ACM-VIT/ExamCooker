...
// Keep the math/diagram/highlighting stack out of ordinary PDF loads.
const PdfQuestionText = dynamic(() => import("./pdf-question-text"), {
  ssr: false,
  loading: () => <span role="status">Formatting question…</span>,
});
          ...
          {paper.questions.map((question, index) => (
            ...
            >
              ...
              <div className="min-w-0 font-serif text-[17px] leading-8">
                <PdfQuestionText>{question.text}</PdfQuestionText>
              </div>
...
function LoadedDocumentSurface({
  ...
  const loadPaper = useCallback(
    async ({
      ...
      if (showPaper) {
        // Overlap the renderer download with the requested Markdown data.
        void import("./pdf-question-text").catch(() => undefined);
